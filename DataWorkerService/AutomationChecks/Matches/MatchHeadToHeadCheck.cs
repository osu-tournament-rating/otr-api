...
/// <see cref="Tournament.TeamLobbySize"/> of 1 where all <see cref="Match.Games"/> were played with a
...
public class MatchHeadToHeadCheck(ILogger<MatchHeadToHeadCheck> logger) : AutomationCheckBase<Match>(logger)
{
    ...
    protected override bool OnChecking(Match entity)
    {
        ...
        if (entity.Tournament.TeamLobbySize != 1)
        {
            logger.LogDebug("Match's tournament team size is not 1 [Team lobby size: {TeamLobbySize}]", entity.Tournament.TeamLobbySize);
            return true;
        }

...
public class MatchesRepository : RepositoryBase<Match>, IMatchesRepository
{
    ...
    public MatchesRepository(ILogger<MatchesRepository> logger, IMapper mapper, OtrContext context,
        IMatchDuplicateRepository matchDuplicateRepository) : base(context)
    {
        _logger = logger;
        _mapper = mapper;
        _context = context;
    ...
    {
        return await MatchBaseQuery(filterInvalidMatches).FirstAsync(x => x.Id == id);
    }

    public async Task<IEnumerable<Match>> GetAsync(IEnumerable<int> ids, bool onlyIncludeFiltered)
    {
        return await MatchBaseQuery(onlyIncludeFiltered).Where(x => ids.Contains(x.Id)).ToListAsync();
    }
    ...
    {
        return await _context.Matches.Where(x => matchIds.Contains(x.MatchId)).ToListAsync();
    }
    ...
    public async Task<int> UpdateVerificationStatusAsync(long matchId, MatchVerificationStatus status,
        MatchVerificationSource source, string? info = null)
    {
        ...
        _logger.LogInformation(
            "Updated verification status of match {MatchId} to {Status} (source: {Source}, info: {Info})", matchId,
            status, source, info);
        return await _context.SaveChangesAsync();
    }
    ...
    public async Task<int> CountMatchWinsAsync(long osuPlayerId, int mode, DateTime fromTime)
    {
        ...
        foreach (var match in matches)
        {
            ...
            {
                if (!game.MatchScores.Any(x => x.Player.OsuId == osuPlayerId)) continue;
            ...
            foreach (var game in match.Games)
                ...
                {
                    _logger.LogWarning(e, "Error occurred while calculating match wins for player {OsuId}",
                        osuPlayerId);
                }

            if (pointsPlayer > pointsOpponent) wins++;
        }
    ...
    {
        if (invalidOnly)
            await _context.Matches.Where(x =>
    ...
    public async Task MergeDuplicatesAsync(int matchRootId)
    {
        ...
        if (root == null) throw new InvalidOperationException($"Failed to find corresponding match: {matchRootId}");

        if (root.IsApiProcessed != true) throw new Exception("All matches must be API processed.");

        if (root.Games.Count == 0) throw new Exception("Root does not contain any games.");

        var totalScores = root.Games.Select(x => x.MatchScores.Count).Sum();
        if (totalScores == 0) throw new Exception("Root has no scores.");

        var duplicateReferences = (await _matchDuplicateRepository.GetDuplicatesAsync(matchRootId)).ToList();
        if (!duplicateReferences.Any()) throw new Exception("Match does not have any detected duplicates.");
        ...
        {
            if (root.TournamentId != duplicate.TournamentId) throw new Exception("Tournament ids must match");

            if (duplicate.IsApiProcessed != true) throw new Exception("All matches must be API processed.");
    ...
    public async Task<IEnumerable<IList<Match>>> GetDuplicateGroupsAsync()
    {
        ...
        var groupedByNameAndDate = await _context.Matches
            .Where(m => m.Name != null && m.StartTime.HasValue &&
        ...
        var duplicatesByName = groupedByNameAndDate
            ...
            {
                Group = g.Where(m1 =>
                    g.Any(m2 => m1 != m2 && Math.Abs((m2.StartTime - m1.StartTime)!.Value.TotalHours) <= 2)).ToList(),
                Count = g.Count()
            })
    ...
    public async Task RefreshAutomationChecks(bool invalidOnly = true)
    {
        ...
        if (invalidOnly) query = query.Where(x => x.VerificationStatus == (int)MatchVerificationStatus.Rejected);

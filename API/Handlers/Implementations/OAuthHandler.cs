...
public class OAuthHandler : IOAuthHandler
{
        ...
        IUserRepository userRepository)
    {
        ...
        _jwtAudience = _configuration["Jwt:Audience"] ?? throw new Exception("Jwt:Audience missing from config");
    ...
    public async Task<OAuthResponseDTO?> RefreshAsync(string refreshToken)
    {
        ...
        switch (issuerRole) {
            case "client":
                ...
                accessToken = GenerateAccessToken(client.ClientId.ToString(), _jwtAudience, client.Scopes, ACCESS_DURATION_SECONDS);
                break;
            case "user":
                ...
                accessToken = GenerateAccessToken(user.Id.ToString(), _jwtAudience, user.Scopes, ACCESS_DURATION_SECONDS);
                break;
            default:
                _logger.LogWarning("Decrypted refresh token issuer does not have a valid role");
                return null;
    ...
    /// <returns></returns>
    private string GenerateRefreshToken(string issuer, string audience, string role, int expirationSeconds = 1_209_600)
    {
        var securityKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_jwtKey));
        var credentials = new SigningCredentials(securityKey, SecurityAlgorithms.HmacSha256);
    ...
    {
        string callbackUrl = _configuration["Auth:ClientCallbackUrl"] ?? throw new Exception("Missing Auth:ClientCallbackUrl in configuration!!");

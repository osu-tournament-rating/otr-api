    ...
    private async Task CheckAndEnqueueOutdatedPlayersAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var player in outdatedPlayers)
        {
            ...
            }))
            {
                ...
                {
                    logger.LogDebug("Skipping player {OsuId} ({Username}) - recently processed (cached)",
                ...
                {
                    logger.LogDebug("Skipping player {OsuId} ({Username}) - already pending",
                        player.OsuId, player.Username);
                    skippedCount++;
                    continue;
                }
                    ...
                    FetchPlatform.Osu))
                {
                    ...
                    logger.LogDebug("Failed to reserve player {OsuId} ({Username}) - race condition",
                        player.OsuId, player.Username);
                    skippedCount++;
                    continue;
                }
        ...
        {
            logger.LogWarning("Player update cycle completed [Enqueued: {EnqueuedCount} | Skipped (Pending): {SkippedCount} | Cached: {CachedCount} | Failed: {FailedCount} | Actual Fetches: {ActualFetchCount}/{MaxMessages}]",
        ...
        {
            logger.LogInformation("Successfully enqueued {EnqueuedCount} player fetch messages [Cached: {CachedCount} | Actual Fetches: {ActualFetchCount}/{MaxMessages}]",

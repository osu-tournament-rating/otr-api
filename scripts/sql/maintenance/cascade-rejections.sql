...
UPDATE matches 
SET rejection_reason = matches.rejection_reason | 128
FROM tournaments t
WHERE matches.tournament_id = t.id 
  AND t.rejection_reason != 0
...
UPDATE games 
SET rejection_reason = games.rejection_reason | 512
FROM matches m
WHERE games.match_id = m.id 
  AND m.rejection_reason != 0
...
UPDATE game_scores 
SET rejection_reason = game_scores.rejection_reason | 8
FROM games g
WHERE game_scores.game_id = g.id 
  AND g.rejection_reason != 0
